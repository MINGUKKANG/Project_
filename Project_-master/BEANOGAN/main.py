...
DEFINE_float(gamma, 0.3, "Hyper parameter of BEGAN")
...
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    op_D = tf.train.AdamOptimizer(learning_rate = lr/2, beta1 = conf.beta1).minimize(D_loss, var_list = D_vars, global_step = global_step)

with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    op_G_VAE = tf.train.AdamOptimizer(learning_rate = lr, beta1 = conf.beta1).minimize(G_loss_VAE, var_list = G_vars, global_step = global_step)

with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    op_G_GAN = tf.train.AdamOptimizer(learning_rate = lr, beta1 = conf.beta1).minimize(G_loss_GAN, var_list = G_vars, global_step = global_step)
...
for i in range(conf.n_epoch):
    ...
    for j in range(total_batch):
        ...
        d_,o_d, g = sess.run([D_loss, op_D, global_step], feed_dict = feed_dict)
    ...
    if i % 1 == 0 or i ==(conf.n_epoch -1):
        t_out_name = "noised_VAE_canvas"
        t_f_name = "noised_Dis_canvas"
        feed_dict = {X_noised: noised_train_data[0:128], phase: False}
        t_out, t_f  = sess.run([X_out, D_fake], feed_dict = feed_dict)
        plot_manifold_canvas(t_out[0:100], 10, type = plot_type, name = t_out_name)
        plot_manifold_canvas(t_f[0:100], 10, type = plot_type, name = t_f_name)

    if i % 1 == 0 or i == (conf.n_epoch -1):
        feed_dict = {X_noised: test_data[0:128], phase: False}
        test_out, test_f = sess.run([X_out, D_fake], feed_dict = feed_dict)
        test_out_name = "test_VAE_canvas" + str(i)
        test_f_name = "test_Dis_canvas" + str(i)
        plot_manifold_canvas(test_out[0:100], 10, type = plot_type, name = test_out_name)
        plot_manifold_canvas(test_f[0:100], 10, type=plot_type, name = test_f_name)
    ...
    print("Epoch: %d    lr_D: %f    lr_G: %f    D_loss: %f    G_loss_vae: %f    G_loss_GAN: %f    \ntime: %d hour %d min %d sec\n"
          %(i, lr_, lr_, d_loss, g_loss_vae, g_loss_gan, hour, min, sec))
