...
DEFINE_integer("n_img_plot", 16, "Number of Images for plotting")
...
DEFINE_float("PT_rate", 0.0, "Weight for Pullaway Term")
DEFINE_float("G_rate", 1/3, "Weight for Fake loss")
...
z_r2f, f_out, ori2fake_loss, z_f2r, r_out, fake2ori_loss, PT_loss, G_loss, D_loss, X_patches = ADGAN.ADGAN(X,X_noised,phase)
...
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    op_G = tf.train.AdamOptimizer(learning_rate = lr, beta1 = conf.beta1).minimize(G_loss, var_list = r2f_vars + f2r_vars)
...
for i in range(conf.n_epoch):
    ...
    if i % 1 == 0 or i ==(conf.n_epoch -1):
        ...
        feed_dict_1 = {X: train_data[0:conf.batch_size], X_noised: train_data[0:conf.batch_size], phase: False}
        feed_dict_2 = {X_noised: test_data[0:conf.batch_size], phase: False}
        f_train_img, r_train_img, patch  = sess.run([f_out, r_out, X_patches], feed_dict = feed_dict_1)
    ...
    print("Epoch: %d    lr_D: %f    lr_G: %f    D_loss: %f\nPVAE_loss: %f    AE_loss: %f    PT_loss: %f    G_loss: %f\ntime: %d hour %d min %d sec\n"
          %(i, conf.lr, conf.lr, l_d, l_rf, l_fr, l_pt, l_g, hour, min, sec))
